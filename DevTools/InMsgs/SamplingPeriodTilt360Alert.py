    ...
    def init_ui(self):
        ...
        def set_config(event):
            try:
                ...
                configuration_array.append(bitstring.pack("uint:24", (sampling_period)))
            ...
            output = "9C"  # AMTYPE
            output += "".join(["%0.2X" % e for e in configuration_array.tobytes()])
        ...
        def request_config(event):
            tty_usb = "/dev/ttyUSB" + w_tty_usb.GetValue()
            ls_send_message_uart("\\x00\\x9C", tty_usb, self)
        ...
        w_load_config.SetStringSelection("Default (30 minutes)")
        load_config(0)

        vertical_pos = vertical_pos + vertical_pos_increment
        vertical_pos = vertical_pos + vertical_pos_increment
        self.SetSize((470, vertical_pos))
        self.SetTitle("SamplingPeriodTilt360AlertCfg")
        self.Centre()
        self.Show(True)
