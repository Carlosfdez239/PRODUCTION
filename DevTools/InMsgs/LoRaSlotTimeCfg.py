...
class Example(wx.Frame):
    ...
    def init_ui(self):
        ...
        def set_config(event):
            ...
            try:
                msg_str = Popen(
                    ["java", "-jar", "jars/LoraSlotTimeCfg.jar", str(slot_time)], stdout=PIPE
                )
                (output, err) = msg_str.communicate()
                msg_str.wait()
            except:
                msgb = wx.MessageDialog(
                    self, "Error executing jars/LoraSlotTimeCfg.jar", "ERROR", wx.OK | wx.ICON_HAND
                )
                msgb.ShowModal()
                return
        ...
        def request_config(event):
            tty_usb = "/dev/ttyUSB" + w_tty_usb.GetValue()
            ls_send_message_uart("\\x00\\x90", tty_usb, self)
        ...
        configs_list = ["Custom", "Default", "Default Test", "30 minutes"]
