...
class Example(wx.Frame):
    ...
    def init_ui(self):
        ...
        def set_config(event):
            ...
            try:
                msg_str = Popen(
                    ["java", "-jar", "jars/GsiChCfg.jar", str(string_protocol), str(num_channels)]
                    + ids_channels,
                    stdout=PIPE,
                )
                ...
                msgb = wx.MessageDialog(
                    self, "Error executing jars/GsiChCfg.jar", "ERROR", wx.OK | wx.ICON_HAND
                )
                msgb.ShowModal()
                return
        ...
        def request_config(event):
            tty_usb = "/dev/ttyUSB" + w_tty_usb.GetValue()
            ls_send_message_uart("\\x00\\x81", tty_usb, self)
