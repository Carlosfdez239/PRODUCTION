    ...
    def init_ui(self):
        ...
        def load_config(event):
            ...
            if config == "Default":
                ...
                w_reserved.SetValue(False)
                w_laser_enabled.SetValue(True)
                w_tilt_enabled.SetValue(True)
        ...
        def set_config(event):
            ...
            try:
                ...
                if w_reserved.GetValue():
                    config_byte = config_byte | 0x20
                if w_laser_enabled.GetValue():
                    config_byte = config_byte | 0x10
                if w_tilt_enabled.GetValue():
        ...
        def request_config(event):
            tty_usb = "/dev/ttyUSB" + w_tty_usb.GetValue()
            ls_send_message_uart("\\x00\\x99", tty_usb, self)
        ...
        w_reserved = wx.CheckBox(pnl, label="Reserved", pos=(20, vertical_pos))
        w_reserved.SetValue(False)
        w_reserved.Bind(wx.EVT_CHECKBOX, set_custom_config)
        vertical_pos += vertical_pos_increment
        w_laser_enabled = wx.CheckBox(pnl, label="LaserEnabled", pos=(20, vertical_pos))
        w_laser_enabled.SetValue(True)
        w_laser_enabled.Bind(wx.EVT_CHECKBOX, set_custom_config)
        vertical_pos += vertical_pos_increment
        w_tilt_enabled = wx.CheckBox(pnl, label="TiltEnabled", pos=(20, vertical_pos))
        w_tilt_enabled.SetValue(True)
        w_tilt_enabled.Bind(wx.EVT_CHECKBOX, set_custom_config)
        vertical_pos += vertical_pos_increment
        w_ch1_enabled = wx.CheckBox(pnl, label="TiltCh1", pos=(40, vertical_pos))
        w_ch1_enabled.SetValue(True)
        w_ch1_enabled.Bind(wx.EVT_CHECKBOX, set_custom_config)
        w_ch2_enabled = wx.CheckBox(pnl, label="TiltCh2", pos=(120, vertical_pos))
        w_ch2_enabled.SetValue(True)
        w_ch2_enabled.Bind(wx.EVT_CHECKBOX, set_custom_config)
        w_ch3_enabled = wx.CheckBox(pnl, label="TiltCh3", pos=(200, vertical_pos))
        w_ch3_enabled.SetValue(True)
