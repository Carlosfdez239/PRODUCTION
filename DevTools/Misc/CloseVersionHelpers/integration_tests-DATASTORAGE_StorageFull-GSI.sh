...
function IT_SetConfig_Default() {
    ...
    $SCR_SEND "\x81\x40" $TTY
    sleep 2
    IT_PrintTimestamp "SET DEFAULT SAMPLING PERIOD"
    $SCR_SEND "\x82\x00\x0E\x10" $TTY
    sleep 2
}

function IT_SetConfig_30Minutes() {
    IT_PrintTimestamp "SET SENSOR 553"
    $SCR_SEND "\x81\x41\x00\x00\x22\x90" $TTY
    sleep 2
